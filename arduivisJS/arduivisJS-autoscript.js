...
function scriptLoopSetup(){
	outlet(0, "void loop(){"+'\n');


}

function scriptMaxSliders(scriptIn){
	for(ii=0;ii<scriptIn;ii++){
		if(scriptIn !== 0){
			outlet(0, "int arduivisSlider"+ii+ " = Serial.parseInt();"+'\n');
		}
	}
	for(oo=0;oo<scriptIn;oo++){
		if(scriptIn !== 0 && oo < scriptIn-1){		
			outlet(0, "Serial.print(arduivisSlider"+oo+"); Serial.print("+spacer+");"+'\n' );
		} 
		else if(oo+1 == scriptIn){
			outlet(0, "Serial.println(arduivisSlider"+oo+");"+'\n');
		}
	}			
}
...
function scriptAnalogPins(amount){
	spacer = '" "';

	for(ii=0;ii<amount;ii++){
		if(amount !== 0){
			outlet(0, "int analogInput"+ii+ " = analogRead("+ii+")"+'\n');
		}
	}	
	for(oo=0;oo<amount;oo++){
		if(amount !== 0 && oo < amount-1){		
			outlet(0, "Serial.print(ardvAnalogRead"+oo+"); Serial.print("+spacer+");"+'\n' );
		} 
		else if(oo+1 == amount){
			outlet(0, "Serial.println(ardvAnalogRead"+oo+");"+'\n');
		}
	}	
}
